""" 
Version 3 of a rule-based agent employs same functions as V3, but implements frame skipping (only analysing every 
third frame) for improved efficiency
"""
# Import packages
...
def make_action(screen, info):
    def calculate_object_position(location_data):
        ...
        object_y_bottom = (
            object_y_top + object_height
        )  # add height as y axes is 0 at top of screen
    ...
    def extract_relevant_blocks(obstacles_locations, mario_location):
        ...
        mario_y_bottom = y_coor[
            1
        ]  # extract mario's bottom y coordinate - i.e., his feet

        for obs in obstacles_locations:
            if (
                obs[0][1] >= mario_y_bottom - MARGIN_RAND_ERR
            ):  # if top of obstacle is below mario's feet
    ...
    def is_obstacle_ahead(mario_data, obs_data):
        ...
        return (
            ...
            (not (mario_y_bottom < obs_y_top or mario_y_top > obs_y_bottom))
            and
    ...
    def is_safe_ahead(mario_data, block_data):
        ...
        return (
            ...
            (
                (obs_y_top - mario_y_bottom < MARGIN_RAND_ERR)
                and (obs_x_left < mario_x_right < obs_x_right - MARGIN_RAND_ERR)
            )
            or
            # this block will be directly below mario if he moves right:
            (
                (obs_y_top - mario_y_bottom < MARGIN_RAND_ERR)
                and (obs_x_left < mario_x_right + MARGIN_PATH_AHEAD < obs_x_right)
            )
        )
    ...
    def best_action(object_locations):
        ...
        above_block_locations, below_block_locations = extract_relevant_blocks(
            block_locations, mario_location
        )
...
for step in range(100000):
    ...
    try:
        action = make_action(obs, info)
    # Failsafe: if something fails, choose a random action
    except:
        action = env.action_space.sample()

    # Skip every 2 frames and perform the same action
    for i in range(3):
        ...
        if MARIO_WORLD_LOCATION_REC.count(MARIO_WORLD_LOCATION_REC[0]) == len(
            MARIO_WORLD_LOCATION_REC
        ):
