...
def make_action(screen, info, step, env, prev_action):
    ...
    def extract_relevant_blocks(block_locations, mario_location):
        ...
        underneath_mario_block_locations = []
        ...
        for block in block_locations:
            if not block[0][1] >= mario_real_y-3:
                ...
                underneath_mario_block_locations.append(block)
                
            #else:
                #print(str(mario_real_y), block[0][1])
        
        return obstacle_block_locations, underneath_mario_block_locations

    def block_in_range(mario_data, obstacle_data):
        location, dimensions, object_name = mario_data
        mario_width, mario_height = dimensions
        
        # get mario coordinates
        mario_x_left, mario_y_top = location
        mario_x_right = mario_x_left + mario_width
        mario_y_bottom = mario_y_top + mario_height

        # extract obstable dimensions and size
        location, dimensions, object_name = obstacle_data
        obs_width, obs_height = dimensions
        
        obs_x_left, obs_y_top = location
        
        obs_x_right = obs_x_left + obs_width
        obs_y_bottom = obs_y_top + obs_height
        
        #print("mario_y_bottom_top", mario_y_bottom, mario_y_top, "obs_y_bottom_top", obs_y_bottom, obs_y_top, "x",  obs_x_left - (mario_x_right))
        return (not (mario_y_bottom + 50< obs_y_top or mario_y_top> obs_y_bottom)) and (-8 < obs_x_left - (mario_x_right) < 10)
    
    def safe_path_ahead(mario_data, block_data):
        location, dimensions, object_name = mario_data
        mario_width, mario_height = dimensions
        ...
        # extract obstable dimensions and size
        location, dimensions, object_name = block_data
        obs_width, obs_height = dimensions
        ...
        return ( (obs_y_top - mario_y_bottom < 3) and (obs_x_left < mario_x_right < obs_x_right - 3) ) or ( (obs_y_top - mario_y_bottom < 3) and obs_x_left < mario_x_right + 5 < obs_x_right)
        
        
    try:
        for obstacle in enemy_locations + extract_relevant_blocks(block_locations, mario_locations[0])[0]:
            if block_in_range(mario_locations[0], obstacle):
    ...
    try:
        safe = False
        for block in extract_relevant_blocks(block_locations, mario_locations[0])[1]:
            if safe_path_ahead(mario_locations[0], block):
                safe = True
    except:
        print("no blocks here")
