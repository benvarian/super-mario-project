...
def make_action(screen, info, step, env, prev_action):
    ...
    def calculate_real_locations(location_data):
        location, dimensions, object_name = location_data
        x, y = location
        width, height = dimensions
        ...
        return (real_x, real_y), (width, height)
    
    def should_jump_obstacle(mario_data, obstacle_data):
        # extract mario dimensions and size
        location, dimensions, object_name = mario_data
        mario_x, mario_y = location
        mario_width, mario_height = dimensions
        
        mario_real_x = mario_x
        mario_real_y = mario_y+mario_height
        
        # extract obstable dimensions and size
        location, dimensions, object_name = obstacle_data
        obs_x, obs_y = location
        obs_width, obs_height = dimensions
        
        obs_real_x = obs_x
        obs_real_y = obs_y+obs_height
        
        return (abs(mario_real_y - obs_real_y) < 30 and abs(mario_real_x+mario_width - obs_real_x) < 20)
    
    def should_jump_obstacle_v2(mario_data, obstacle_data):
        # extract mario dimensions and size
        location, dimensions, object_name = mario_data
        mario_x, mario_y = location
        mario_width, mario_height = dimensions
        ...
        return (not (mario_real_y < obs_y or mario_y > obs_real_y)) and (obs_real_x - (mario_real_x+mario_width) < 5)
    
    def is_enemy_close(mario_data, obstacle_data):
        # extract mario dimensions and size
        location, dimensions, object_name = mario_data
        mario_x, mario_y = location
        mario_width, mario_height = dimensions
        ...
        return (not (mario_real_y < obs_y or mario_y > obs_real_y)) and (obs_real_x - (mario_real_x+mario_width) < 3) 
    
    def extract_relevant_blocks(block_locations, mario_location):
        obstacle_block_locations = []
        
        mario_real_coor, mario_real_dim = calculate_real_locations(mario_location)
        mario_real_x, mario_real_y = mario_real_coor
        
        for block in block_locations:
            if not block[0][1] >= mario_real_y-3:
                obstacle_block_locations.append(block)
            #else:
                #print(str(mario_real_y), block[0][1])
        
        return obstacle_block_locations
        
        
    print("START")
    for obstacle in (enemy_locations + extract_relevant_blocks(block_locations, mario_locations[0])):
        print(obstacle)
        if obstacle[2] == 'goomba' and is_enemy_close(mario_locations[0], obstacle):
            return 5
        if should_jump_obstacle_v2(mario_locations[0], obstacle):
            return 2
    ...
    if enemy_locations:
        mario_real_x = mario_x
        mario_real_y = mario_y+mario_height
        if abs((mario_y+mario_height) - (enemy_locations[0][0][1]+enemy_locations[0][1][1])) <10 and abs(mario_x+mario_width - enemy_locations[0][0][0]) < 10:
            return 2
    return 1
    '''
'''
...
for step in range(100000):
    if obs is not None:
        ...
        action = env.action_space.sample()
    #obs, reward, terminated, truncated, info = env.step(int(input()))

    obs, reward, terminated, truncated, info = env.step(action)
    print("ACTION", str(action))
    
    if(obs is not None and LAST_WORLD_Y is not None):
        if(LAST_WORLD_X == info["x_pos"] and LAST_WORLD_Y == info["y_pos"]):
            #"I AM IN SAME POSITION"
            obs, reward, terminated, truncated, info = env.step(1)
        if(LAST_WORLD_Y < info["y_pos"]):
            obs, reward, terminated, truncated, info = env.step(1)
        
    LAST_WORLD_X = info["x_pos"]
    LAST_WORLD_Y = info["y_pos"]
